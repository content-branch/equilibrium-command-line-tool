...
import { EQ_CURRENT_APP } from '../../properties';
...
export default class AppsCurrent extends ConfiguredCommand {
  ...
  static examples = ['eq apps:current -a ckm1w4vy857869go3nsw4mk2ay'];
  ...
  async command() {
    ...
    this.setConfig(EQ_CURRENT_APP, appId);

    const data = await getApp(this.client, appId);
    this.log(`Updated property ${EQ_CURRENT_APP}`);
